...
export default async function LibraryPage() {
	...
	const [{ data: students }, { data: subjects }, { data: memberships }] = await Promise.all([
		supabase
			.from("students")
	...
	// a role that carries library.manage (simple RLS read — the API re-checks
	...
	if (!canManage && roles.length > 0) {
		const { data: perms } = await supabase
			...
			.eq("permission_key", "library.manage");
